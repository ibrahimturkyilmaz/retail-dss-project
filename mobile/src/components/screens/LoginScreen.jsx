...
import { ShoppingBag, ArrowRight, Mail, User, X, Sparkles } from 'lucide-react';
...
export default function LoginScreen({ onLogin }) {
    ...
    const [googleStep, setGoogleStep] = useState(1); // 1: email, 2: confirm
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-gray-900">Google ile Giriş Yap</h3>
                                        <p className="text-xs text-gray-400">
                                            {googleStep === 1 ? 'Adım 1/2 — Hesap Seçimi' : 'Adım 2/2 — Onay'}
                                        </p>
                                    </div>
                                </div>
                                <button onClick={() => setShowGoogleModal(false)} className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-200">
                                    <X size={16} />
                                </button>
                            </div>
                            ...
                            <div className="flex gap-2 mb-6">
                                <div className={`h-1 flex-1 rounded-full ${googleStep >= 1 ? 'bg-indigo-600' : 'bg-gray-200'}`} />
                                <div className={`h-1 flex-1 rounded-full ${googleStep >= 2 ? 'bg-indigo-600' : 'bg-gray-200'}`} />
                            </div>
                            ...
                            <AnimatePresence mode="wait">
                                {googleStep === 1 ? (
                                    ...
                                    >
                                        <div className="space-y-1.5">
                                            <label className="text-sm font-semibold text-gray-700 ml-1">Gmail Adresiniz</label>
                                            <div className="relative">
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full bg-gray-50 border border-gray-100 rounded-2xl py-3.5 pl-11 pr-4 text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium"
                                                    placeholder="Google hesap adınız"
                                                />
                                            </div>
                                        </div>
                                    </motion.div>
                                    ...
                                    >
                                        ...
                                        <div className="bg-gray-50 rounded-2xl p-4 flex items-center gap-4 border border-gray-100">
                                            <div className="w-14 h-14 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xl font-bold shadow-lg shadow-indigo-200">
                                                {(googleName || googleEmail)[0]?.toUpperCase()}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <p className="font-bold text-gray-900 truncate">{googleName || 'Google Kullanıcı'}</p>
                                                <p className="text-sm text-gray-500 truncate">{googleEmail}</p>
                                            </div>
                                        ...
                                        <p className="text-sm text-gray-500 text-center">
                                            Bu hesapla giriş yapmak istediğinize emin misiniz?
                                        </p>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                            ...
                            <div className="flex gap-3 mt-6">
                                {googleStep === 2 && (
                                    <button
                                        ...
                                        className="flex-1 bg-gray-100 text-gray-600 rounded-2xl py-3.5 font-bold hover:bg-gray-200 transition-colors"
                                    >
                                        Geri
                                    </button>
                                )}
                                <motion.button
                                    ...
                                    className="flex-1 bg-indigo-600 text-white rounded-2xl py-3.5 font-bold shadow-lg shadow-indigo-200 flex items-center justify-center gap-2 hover:bg-indigo-700 transition-colors disabled:opacity-50"
                                >
                                    {isLoading ? (
